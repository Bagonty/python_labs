...
COIN_VALUES = [1, 3, 5]        # Coins with different weights
COINS_FOR_SPEED = 10           # Enemy speed increases after every N collected coin points
MAX_SPEED = 12                 # Maximum game speed
...
class PlayerCar(pygame.sprite.Sprite):
    ...
    def update(self):
        """Move player car inside road borders."""
...
class EnemyCar(pygame.sprite.Sprite):
    ...
    def update(self):
        """Move enemy down and delete it when it leaves the screen."""
...
class Coin(pygame.sprite.Sprite):
    """
    Coin with different weight/value.
    ...
    def __init__(self, speed):
        ...
        # Draw value inside coin
        font = pygame.font.SysFont("Arial", 14, bold=True)
        label = font.render(str(self.value), True, BLACK)
        self.image.blit(label, label.get_rect(center=(14, 14)))

        self.rect = self.image.get_rect()
        self.rect.x = random.randint(ROAD_LEFT, ROAD_RIGHT - 28)
        self.rect.y = -30

        self.speed = speed

    def update(self):
        """Move coin down and delete it when it leaves the screen."""
        self.rect.y += self.speed
...
class RoadStripe:
    """Animated road center stripe."""
    def __init__(self):
        self.y = 0
        self.gap = 60

    def update(self, speed):
        """Move road stripe down cyclically."""
        self.y = (self.y + speed) % self.gap

    def draw(self, surface):
        """Draw dashed white line in the center of the road."""
        x = SCREEN_WIDTH // 2 - 4
        y = self.y - self.gap
...
def draw_road(surface):
    """Draw road background and road borders."""
    surface.fill(GRAY)
...
def draw_hud(surface, font, score, coins_collected, speed):
    """Draw score, collected coins and current speed."""
    ...
    speed_text = font.render(f"Speed: {speed}", True, GREEN)
...
def main():
    ...
    # Needed to avoid increasing speed many times on the same coin amount
    next_speed_boost = COINS_FOR_SPEED
    ...
    while True:
        ...
        if not game_over:
            ...
            # Update sprites and road stripe
            all_sprites.update()
            ...
            # Each collected coin can have different value
            for coin in collected:
                coins_collected += coin.value

            # Practice 11 requirement:
            # increase enemy speed when player earns N coins
            if coins_collected >= next_speed_boost:
                scroll_speed = min(scroll_speed + 1, MAX_SPEED)
                next_speed_boost += COINS_FOR_SPEED
        ...
        draw_hud(screen, font, score // 10, coins_collected, scroll_speed)
